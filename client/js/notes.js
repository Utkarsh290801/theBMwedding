...
const venueAvailabilityGroup = document.getElementById(
  "venueAvailabilityGroup",
);
...
function getGuestEventValues(note = null) {
  ...
  return Array.from(
    guestEventOptions.querySelectorAll("input[type='checkbox']:checked"),
  )
...
function updateStats() {
  ...
  venueCount.textContent = notes.filter((n) => n.category === "Venue").length;

  shoppingCount.textContent = notes.filter(
    (n) => n.category === "Shopping",
  ).length;

  budgetCount.textContent = notes.filter((n) => n.category === "Budget").length;
}
  ...
  const filtered = notes.filter((note) => {
    ...
    const matchesStatus =
      ...
      (selectedStatus === "Shortlisted"
        ...
        : note.category === "Venue" &&
          note.venueAvailability === selectedStatus);

    return (
      matchesSearch && matchesCategory && matchesGuestEvent && matchesStatus
    );
  });
...
saveButton.addEventListener("click", async () => {
  ...
  const venueAvailability =
    category === "Venue" ? venueAvailabilityInput.value : undefined;
  const shortlisted = category === "Venue" ? shortlistsInput.checked : false;
  const guestCount =
    category === "Guests" ? Number(guestCountInput.value) : undefined;
  const guestEvents = category === "Guests" ? getGuestEventValues() : [];
